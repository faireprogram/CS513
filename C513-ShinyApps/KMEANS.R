...
# age<-Training_ECG[,1]
# cholestrol<-Training_ECG[,5]
# 
# Predict_2_ECG <- kmeans(Training_ECG[,-9], 3)
# Predict_2_ECG
# #Table created to see number of values in each custer
# table(Training_ECG[,9],Predict_2_ECG$cluster)
# #Plotting on the bases of cluster


lvars$KMEAN_DATASET<-Training_ECG[,c(input$xcol,input$ycol)]

lvars$KMEAN_CLUSTER <- kmeans(lvars$KMEAN_DATASET, input$Kmean_K)
