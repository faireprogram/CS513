...
(function( $ ) {
    ...
    if (typeof(Shiny) != "undefined") {
      (function() {
        ...
        function onLoggedIn(credentials) {
          ...
          div.find('a').attr('href', '__logout__');
          $('body').append(div);
        }
       ...
       // var token = xhr.responseText;
          ...
          Shiny.createSocket = function() {
            try {
              if (window.parent.ShinyServer && window.parent.ShinyServer.multiplexer) {
                var relURL = getRelativePath(
                  window.parent.ShinyServer.url, sockjsUrl, true);
        ...
        setInterval(function(){jQuery.ajax("__extendsession__", {type:"POST", async: true});},5*60*1000);
        ...
        exports.multiplexer = new MultiplexClient(conn, sockjsUrl);
        ...
        Shiny.oncustommessage = function(message) {
          ...
          if (message.credentials) onLoggedIn(message.credentials);
  ...
  // another channel.
  function MultiplexClient(conn, url) {
    // The underlying SockJS connection. At this point it is not likely to
    // be opened yet.
    this._conn = conn;
    // The base URL of the SockJS connection. This information isn't
    // available from the Go SockJS library so we need to pass it ourselves.
    this._url = url;
